...
def recibo():
    ...
    fecha_recibo = f'{fe.day}/{fe.month}/{fe.year} - {fe.hour}:{fe.minute}'
    ...
    texto_recibo.insert(END, f' Costo de la Comida: \t\t\t{
        var_costo_comida.get()}\n')
    texto_recibo.insert(END, f' Costo de la Bebida: \t\t\t{
        var_costo_bebida.get()}\n')
    texto_recibo.insert(END, f' Costo de la Postres: \t\t\t{
        var_costo_postres.get()}\n')
    texto_recibo.insert(END, '-' * 54 + '\n')
...
etiqueta_titulo = Label(panel_superior,
                        text='Sistema de Facturacion', fg='azure4',
